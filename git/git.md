...
Config- файл в текущей директории с репозиторием &lt;repo&gt;/.git/config

Global файл в домашней директории пользователя ~/.gitconfig

System файл настроек для всех пользоватаелей /etc/gitconfig

git config --list Посмотреть лист всех настрaоек

git config --list --local Также можно ограничить конкретным уровнем
...
git config --system core.editor <editor>Текстовый редактор для всех пользователей. Открывается в момент формирования сообщения о комите.Можно также его указать через переменные окружения GIT_EDITOR, EDITOR, или VISUAL

git config --global --edit  Открыть файл для редактирования
...
Можно добавить ссылку на сложную команду и последовательность команд

$ git config --global alias.cp cherry-pick

Или даже составную. Выглядет это в файле конфигураций так
...
$ git config --global help.autocorrect 10  Указывается сколько десятых секунды нужно подождать прежде чем применить скорректированную команду. Можно отменить через Ctrl + C
...
Если проект уже есть и необходимо создать репозиторий в папке проекта.

git init &lt;directory&gt;

Команда создает директорию и создаёт пустой git репозиторий в указанной папке. В этой директории будет создана папка .git.
...
Инициализирует пустой репозиторий не предполагающий наличие содержимого в текущей папке. Делается для серверного репозитория.
...
Получение кода из удалённого репозитория. Clone.

$ git clone git://github.com/schacon/grit.git

При этом будет создан новый каталог grit

git clone &lt;repo&gt; <directory

$ git clone git://github.com/schacon/grit.git mygrit

При этом будет создан новый каталог mygrit
...
git add -p Добавление порции изменений
...
commit -m "&lt;message&gt;"  передача описания без открывания редактора
...
git commit -v Добавляет в сообщение diff изменений
...
1\.  git add (with various options) зафиксировать изменения в индексе
...
Если комит поменяли,  но потом поняли, что что-то неправильно. Можно посмотреть

git log -g   это так называемый reflog
...
Если хотим не поменять последний комит а вообще отменить его. Команда отменяет последний комит и меняет working tree и индекс. В отличие от git reset которая сбрасывает только индекс. Точнее команда смещает указатель HEAD. При этом комит остается в истории (локальной)

Но его также можно вернуть через reflog

git reset HEAD~ можно также git HEAD~0
