...
echo some_text
```
Опция `-e` позволяет писать дальше в кавычках
```bash
echo `-e` "some texts"
```
управляющие символы: \b backspace \n newline \t tab
...
символы: %s строка %d целое числов %f дробное число. Для дробных можно указывать точность вот так например с одним разрядок в дробной части
```bash
printf "%.1f\n" 255
```
