...
Многие команды позволяют использовать аргументы с wildcard символами. Если не понятно что это, почитайте про regexp. 
```
ls host*
...
## grep
Команда поиска строки по условию
...
```
-l отображает не содержимое, а имя файла
...
В этом режиме права доступа задаются так chmod groupoperatorpermission
...
setfacl -R -m g:name\_of\_group:permissona directoryУстановить правило
...
setfacl -m d:g:name\_of\_group:permissona directoryУстановить правило (default ACL). Как сказано выше, это правило будет применяться для всех новых файлов внутри директории
...
su - usernameКоманда позволяет запустить shell другого пользователя
...
rpm -qf file_nameПонять в каком пакете находился тот или иной файл
...
rpm -ql nameСписок файлов в пакете

rpm -qс nameСписок конфигурационных файлов в пакете

rpm -qd nameСписок файлов документации в пакете

rpm -qp --scripts nameПоказать содержимое скриптов в пакете
...
yum module provides nameПоиск модуля, который содержит пакет

yum module info nameИнформация о модуле

yum module info --profile nameПоиск профилей

![](images/image382.png)

yum module list nameОтображение stream внутри модуля
